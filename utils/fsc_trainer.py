...
def train_collate(batch):
    ...
    ex_list = torch.stack(transposed_batch[2], 0)
    # ex_list0 = torch.stack(transposed_batch[2], 0)
    # ex_list1 = torch.stack(transposed_batch[3], 0)
    # ex_list2 = torch.stack(transposed_batch[4], 0)
    # ex_list = transposed_batch[2]
    return images, dmaps, ex_list
    # return images, dmaps, ex_list0, ex_list1, ex_list2
...
class FSCTrainer(Trainer):
    ...
    def train_epoch(self):
        ...
        # for inputs, targets, ex_list0, ex_list1, ex_list2 in tqdm(self.dataloaders['train']):
            inputs = inputs.to(self.device)
            targets = targets.to(self.device) * self.args.log_param
            # ex_list0 = ex_list0.to(self.device)
            # ex_list1 = ex_list1.to(self.device)
            # ex_list2 = ex_list2.to(self.device)
            ex_list = ex_list.to(self.device)
            ...
            with torch.set_grad_enabled(True):
                # et_dmaps = self.model(inputs, ex_list0, ex_list1, ex_list2)
    ...
    def val_epoch(self):
        ...
        # for inputs, count, ex_list0, ex_list1, ex_list2, name in tqdm(self.dataloaders['val']):
            ...
            if h > max_size or w > max_size:
                ...
                with torch.set_grad_enabled(False):
                    ...
                    for input_ in input_list:
                        # output = self.model(input_, ex_list0, ex_list1, ex_list2)
                        output = self.model(input_, ex_list)
                        pre_count += torch.sum(output) 
            else:
                with torch.set_grad_enabled(False):
                    # output = self.model(inputs, ex_list0, ex_list1, ex_list2)
